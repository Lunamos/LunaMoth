...
- [x] **Messaging gateway (WeCom + personal WeChat)** — `lunamoth gateway NAME` runs one activated chara behind `~/.lunamoth/sessions/NAME/messaging.json`; adapters deliver only `say` text (including idle `speak` output) and drop muse/thinking/tool chatter. WeCom/Enterprise WeChat self-built apps use the stdlib callback server plus callback crypto from `uv sync --extra messaging`.
