...
_TOOL_RESULT_CLIP = 400      # truncate long tool outputs when summarizing the head
...
def _serialize(messages: list[dict]) -> str:
    """Flatten the head into plain text for the summarizer (tool noise clipped)."""
    out: list[str] = []
    for m in messages:
        ...
        if m.get("kind") == "summary":
            ...
            clipped = content[:_TOOL_RESULT_CLIP] + ("…" if len(content) > _TOOL_RESULT_CLIP else "")
            out.append(f"[tool result] {clipped}")
...
def compact(ctx: ContextBuffer, lang: str, llm, *, force: bool = False) -> bool:
    ...
    ctx.messages = [summary_msg] + msgs[cut:]
    return True
