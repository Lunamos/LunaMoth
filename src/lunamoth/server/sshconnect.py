"""`lunamoth connect ssh://[user@]host[:port]` — reach a remote chara over an SSH tunnel.

The cleanest secure remote path (plan §3 Track D, §7): the remote `lunamothd`
...
def parse_daemon_json(text: str) -> RemoteDaemon:
    """Parse the remote `~/.lunamoth/daemon.json` text into a RemoteDaemon.
    ...
    if not token or not http_port or not ws_port:
        raise ConnectError(
            "remote daemon.json missing token/http_port/ws_port — is lunamothd running on the remote?"
        )
...
def read_remote_daemon(target: SshTarget) -> RemoteDaemon | None:
    ...
    (so the caller can decide to start lunamothd); raises ConnectError on an SSH
    failure (auth/host) or a malformed-but-present file."""
    rc, out, err = _ssh_exec(target, "cat ~/.lunamoth/daemon.json")
    if rc != 0:
...
def start_remote_daemon(target: SshTarget) -> None:
    """Start lunamothd on the remote (`lunamoth desktop --daemon`). Raises on failure."""
    rc, out, err = _ssh_exec(
        target, "lunamoth desktop --daemon --no-open", timeout=60.0
    )
    if rc != 0:
        detail = (err or out or "").strip() or f"ssh exited {rc}"
        raise ConnectError(
            f"could not start lunamothd on {target.target()}: {detail}\n"
            "       Start it manually there (`lunamoth desktop --daemon`) and retry."
        )


def ensure_remote_daemon(target: SshTarget, *, log=print) -> RemoteDaemon:
    """Make sure lunamothd is up on the remote and return its daemon info.
    ...
    if daemon is not None:
        return daemon
    log(f"no lunamothd on {target.target()} — starting it…")
    ...
    for _ in range(10):
        ...
        time.sleep(1.0)
    raise ConnectError(
        f"started lunamothd on {target.target()} but its daemon.json never appeared."
    )
