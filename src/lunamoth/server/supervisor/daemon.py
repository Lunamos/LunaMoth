"""Daemon (lunamothd) metadata: the daemon.json contract + liveness/stop.

Pure stdlib + the session home; no asyncio, no HTTP. ``lunamoth start`` /
``--connect`` / ``lunamoth daemon`` read these to find and control the resident
supervisor.
"""
