...
Storage: ``~/.lunamoth/auth.json`` holds ONLY ``{algo, iters, salt, hash}`` —
...
def auth_store_path() -> Path:
    """``~/.lunamoth/auth.json`` — the PBKDF2 password record (hash+salt only)."""
    return S.lunamoth_home() / "auth.json"
...
def ensure_password(
    *, env_password: str | None, path: Path | None = None
    ...
    Precedence:
      1. ``env_password`` (``LUNAMOTH_PASSWORD``) — re-hashed + stored every run
    ...
    if env:
        if len(env) < _MIN_LENGTH:
            raise ValueError(
                "LUNAMOTH_PASSWORD must be at least 8 characters (refusing an empty/weak password)"
            )
