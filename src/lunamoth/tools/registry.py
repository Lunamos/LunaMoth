"""Central registry for all LunaMoth builtin tools — ported from hermes-agent
(reference/hermes-agent/tools/registry.py), apple-to-apple, adapted to
LunaMoth's runtime in two ways only:
...
logger = logging.getLogger("lunamoth.tools.registry")
...
def discover_builtin_tools(tools_dir: Optional[Path] = None) -> list[str]:
    ...
    if not base.is_dir():
        return []
    names = [
        f"lunamoth.tools.builtin.{path.stem}"
...
class ToolRegistry:
    ...
    def dispatch(self, name: str, args: dict, ctx) -> str:
        ...
        raises into the turn (LunaMoth's gateway adds the audit + loop guard on
        top of this)."""
